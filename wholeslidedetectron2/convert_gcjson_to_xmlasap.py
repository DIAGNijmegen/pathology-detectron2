...
def collect_arguments():
    argument_parser = argparse.ArgumentParser(
        description="Convert gc json points to asap xml points"
    )
    ...
    return arguments["input_path"], arguments["output_path"]
...
def main(input_path, output_path):
    ...
    for idx, point in enumerate(data['points']):
        point = point['point']
        p = Point(idx, None, label, [(point[0], point[1])])
        points.append(p)
    write_asap_annotation(points, output_path)
    

if __name__ == "__main__":
    input_path, output_path = collect_arguments()
